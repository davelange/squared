export const SquarePass = {
  ...
  fragmentShader: /* glsl */ `
    ...
    void main() {
        ...
        vec2 dist = vec2(roundedBoxSDF(fractalUv, vec2(size), 0.1));        


        vec2 final = dist;        
        final *= abs(cos(scale * p.yx + 1.23 * timeSlow * vec2(2.2,3.4)) + cos(scale * p.xy + 1.23 * timeSlow * vec2(2.2,3.4)));
        ...
        float distColor = 1. - smoothstep(0.01, 0.01, final.x);
